function separateByAllStimuliFig(expt,spikes,unitTag,fileInd,b,saveTag)
...
disp('Making figure with spikes separated by each stimulus');
...
if isempty(saveTag)
    saveTag = [unitTag 'separateByAllStimuli'];
else
...
if ~isempty(spikes.spiketimes) % Check that there are spikes
    ...
    if length(expt.stimulus(fileInd(1)).varparam)>1
        nDiffStimuli=length(expt.stimulus(fileInd(1)).varparam(1).Values)*length(expt.stimulus(fileInd(1)).varparam(2).Values);
    else
        nDiffStimuli=length(expt.stimulus(fileInd(1)).varparam(1));
    end
    stim.type = 'all';
    stim.values = 1:nDiffStimuli;
...
% 	if strcmp(cond.type,'all')
    ...
    if sum(spikes.fileInd==fileInd(1))>5000
    ...
    h=plotAvResponsesForWindows(h,'cr',cspikes,allfr.stim,nallfr.stim,stim,cond,'Stim','Stim Cond');
    h=plotAvResponsesForWindows(h,'crOn',cspikes,allfr.on,nallfr.on,stim,cond,'Stim Onset','Stim Cond');
    h=plotAvResponsesForWindows(h,'crOff',cspikes,allfr.off,nallfr.off,stim,cond,'Stim Offset','Stim Cond');
    h=plotAvResponsesForWindows(h,'crLedOn',cspikes,allfr.ledOn,nallfr.ledOn,stim,cond,'LED On','Stim Cond');
    h=plotAvResponsesForWindows(h,'crSpont',cspikes,allfr.spont,nallfr.spont,stim,cond,'Spont','Stim Cond');
...
s2 = 0.03;     % Space between rows
s3 = 0.005;      % Space between columns
