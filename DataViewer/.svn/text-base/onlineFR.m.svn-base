...
for m = 1:size(h.frData,1)
    for n = 1:size(h.frData,2)
        ...
        h.lines(m,n) = line('Parent',h.axs(k),'Color',colors{n},'Visible','off', ...
...
frOn(h,dvplot.rvOn,ismember(pvOnOrdered,dvplot.rvOn));
...
function frOn(h,k,k2)
set(h.axs(k),'Visible','on')
set(h.lines(k2,:),'Visible','on')
