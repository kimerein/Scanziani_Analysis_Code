...
% KR check if stimType is in expt.stimulus or expt.files
if ~isfield(expt.stimulus(fileInd).params,'stimType')
    if ~isfield(expt.files,'stimType')
        stimType='Flash';
    else
        stimType=expt.files.stimType{fileInd};
    end
else
    stimType = expt.stimulus(fileInd).params.stimType;
end
if ~isfield(expt.stimulus(fileInd).params,'delay')
    expt.stimulus(fileInd).params.delay=0.5;
    delay=0.5;
else
    delay = expt.stimulus(fileInd).params.delay;
end
if ~isfield(expt.stimulus(fileInd).params,'duration')
    expt.stimulus(fileInd).params.duration=1;
end
