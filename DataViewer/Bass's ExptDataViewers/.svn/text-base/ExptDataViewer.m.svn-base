...
rigDefs = RigDefs;
Thresholds = min(cell2mat(get(handles.hThresh(2:17),'Ydata'))');
expt = handles.expt;
expt.sort.manualThresh = Thresholds;
assignin('base','expt',expt);
save(fullfile(rigDefs.Dir.Expt,expt.info.exptfile),'expt');
...
dvProcessDisplay(dv,data, handles.hDataViewer);

% add unit spikes
 dvUnitDisplay(dv,handles.expt,PlotObj,handles.hDataViewer)
