...
for i = 1:length(dvHandles.Channel)
...
for m = 1:size(h.lfpData,1)
    for n = 1:size(h.lfpData,2)
        k=pvOnOrdered(m); % KR fixed bug 2/27/11
        tempColor = get(dvHandles.hPlotLines(k),'Color');
        h.lines(m,n) = line('Parent',h.axs(k),'Color',tempColor,'Visible','off', ...
...
% Make LFPs visible
lfpOn(h,h.rvOn,ismember(pvOnOrdered,dvplot.rvOn));  % 2/27/11 KR fixed bug

% Add ticks
for i = 1:nPlotOn
    if showAllYAxes
        % Put 2 ticks on y-axis
        k=pvOnOrdered(i); % KR fixed bug 2/27/11
        setAxisTicks(h.axs(k));
    end
end
...
function lfpOn(h,k,k2)
set(h.axs(k),'Visible','on')
set(h.lines(1:end,:),'Visible','on')
