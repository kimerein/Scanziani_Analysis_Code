...
if strcmp(expt.probe.type,'16 Channel') || strcmp(expt.probe.type,'16 channel')  
...
switch expt.probe.configuration
    case {'2x2','2 x 2'}
        if ~isTDTexpt(expt)
            % 
            expt.probe.channelorder = RigDef.ChannelOrder{1};
        else         % for TDT same but they are flipped in order already so
            expt.probe.channelorder = [1:16];
        end
    case {'1x16','1 x 16','1X16'}
        expt.probe.channelorder = RigDef.ChannelOrder{2};
    case {'4x1','4 x 1'}
