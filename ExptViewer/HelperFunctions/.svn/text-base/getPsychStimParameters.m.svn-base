...
% Need these variables for KR hack, because my stim. computer receives
% "UPDATEDATA" as stim. file when stim. file is same as stim. file for
% previous .daq file
persistent lasttemp % KR hack
...
if ~bdebugParam & ~isempty(dir([ftemp '.*']))
    ...
    % KR hack for when receive "UPDATEDATA" as stim. file name - means
    % stim. file has not changed from previously acquired .daq file
    if strcmp(temp,'UPDATEDATA');
        disp(fname);
        disp('RECEIVED UPDATEDATA FROM STIM COMPUTER');
        temp=lasttemp;
    else
        lasttemp=temp;
    end
...
if vStimCtlr
    ...
    % KR - added next 3 lines for compatability with MakeExptSRO.m
    Param.delay=PreWaitInt;
    Param.duration=Duration;
    Param.wait=WaitInterval;
    if blankbkgrnd
...
disp(fullfile(DAQFILEPATH,sfilename))
...
i =1;
% KR - fixed bug; user input is not a string
for j=1:nToConsider+1
    if ui==j
        i=j;
    end
end
