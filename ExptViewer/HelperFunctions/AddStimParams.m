...
for i = 1:length(expt.files.names)
    ...
    % KR - if certain stim parameters not defined by PsychStimController, use defaults in RigDef
    if ~isfield(expt.stimulus(i).params,'delay') 
        expt.stimulus(i).params.delay=RigDef.Daq.TimeBeforeStimOnset;
        expt.stimulus(i).params.duration=RigDef.Daq.StimDuration;
        expt.stimulus(i).params.wait=RigDef.Daq.TimeBetweenStimOffsetAndNextTrial;
    end
