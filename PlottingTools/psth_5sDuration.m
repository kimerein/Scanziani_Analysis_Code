function [varargout] = psth_5sDuration(spikes,binsize,hAxes,bsmooth)
...
duration = 6.5;     % Maybe add checking for equal sweep durations?
...
hold on; 
plot(centers(1:end-1),n(1:end-1)-2*std(nsForStdev(1:end-1),1),'Color','green');
plot(centers(1:end-1),n(1:end-1)+2*std(nsForStdev(1:end-1),1),'Color','green');