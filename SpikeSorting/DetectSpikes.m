...
%   4/12/10 - SRO
%   6/30/10 - SRO
%4/11/2011 - KR compatability with Kim's rig
...
for fileInd = detectFiles
    ...
    if isTDTexpt(expt)
        ...
        if fileInd == detectFiles(1)
            ...
            dataTemp = [];
            % Fix for KR's shitty memory machine
...
%                     dataTemp2 = [dataTemp; data];
%                 end
                for i = 1:length(sampFiles)
                    disp(['Reading in ' num2str(i) ' of ' num2str(length(sampFiles)) ' files...']);
                    %data = daqread([RigDef.Dir.Data FileList{fileInd}],'Channels',channels(j)+1);
        ...
        disp(FileList{fileInd})
        %data = daqread([RigDef.Dir.Data
        %FileList{fileInd}],'Channels',channels+1);    % Recording channels are offset by 1 because trigger is channel 1
...
save(fullfile(RigDef.Dir.Spikes,getFilename(spikes.info.spikesfile)),'spikes','-v7.3');
