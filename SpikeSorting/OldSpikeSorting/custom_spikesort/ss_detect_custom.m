function spikes = ss_detect_custom( data, spikes, stds )
...
if append % use old threshold
   ...
   % I use MAD estimate of SD in DetectSpikes - SRO
    if isequal( spikes.params.detect_method, 'auto' )
        thresh = stds * -params.thresh
    elseif isequal( spikes.params.detect_method, 'manual' )
        thresh = params.thresh;
        disp('Using manually set thresholds')
    else
