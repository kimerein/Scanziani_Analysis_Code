    ...
    elseif length(figdata.selected) ==  length(setdiff( unique(figdata.tempassigns), 0 ) )
        ...
        for j = 1:length(maps)
            if ismember( maps(j).to, figdata.selected )
                figdata.spikes = split_cluster(figdata.spikes,maps(j).from);
            end
        end
...
function break_microcluster( varargin )
    ...
    [figdata.spikes,newclust] = break_minicluster(figdata.spikes,figdata.selected);
