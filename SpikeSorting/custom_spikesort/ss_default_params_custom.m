...
%
    ...
    spikes.params.thresh = 3;             % for 'auto', set number of standard deviations above background noise
 %  spikes.params.thresh =    [-5.3897 -5.0999 -5.1816 -5.5274]; % for 'manual', set specific thresholds for each channel
    ...
    spikes.params.agg_cutoff = .0001;       %  higher = less aggregation, lower = more aggregation
    spikes.params.kmeans_clustersize = 1000; %  target size for clusters
    ...
    spikes.params.initial_split_figure_panels = 4;
