...
stronglySup=((X(:,6)>2.4*10^-4)&(X(:,1)./X(:,4)>0.75));
otherSup=((X(:,6)>2.4*10^-4)&(X(:,1)./X(:,4)<=0.75));
...
colors(reallyFat)=2;
colors(otherSup&~reallyFat)=6;
%colors=zeros(size(X,1),1)+stronglySup+2*stronglyFac;

figure();
scatter(X(:,3),X(:,2),[],colors);
xlabel('F1');
ylabel('effectDelay');
...
figure();
scatter(X(:,1)./X(:,4),X(:,9)./X(:,4),[],colors);
xlabel('fractional sup');
